...
std::vector<double> TemplateEEController::getInputs(){
    ...
    // - normalized energy levestepEvolution();l (if any)
...
void TemplateEEController::createNN()
{
    ...
    switch ( TemplateEESharedData::gControllerType )
    {
	    ...
	    std::cout<< _nbHiddenLayers << ", " << (*_nbNeuronsPerHiddenLayer)[0] << std::endl;
            nn = new MLP(_parameters, _nbInputs, _nbOutputs, *(_nbNeuronsPerHiddenLayer), true);
            break;
        }
        case 1:
        {
...
{
    _currentSigma = sigma;
    //std::cout << "mutateGaussian" << std::endl;
    for (unsigned int i = 0 ; i != _currentGenome.size() ; i++ )
...
void TemplateEEController::initEvolution()
{
    ...
    for ( unsigned int i = 0 ; i != nbGene ; i++ )
    {
        ...
        //double tmp = ((random01()*2)-1) *TemplateEESharedData::gNeuronWeightRange/2 ;
        //std::cout<<tmp<<"\n";
        //std::cout<<TemplateEESharedData::gNeuronWeightRange/2 * tmp<<"\n";
