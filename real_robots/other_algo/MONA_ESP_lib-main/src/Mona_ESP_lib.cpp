...
//IR sensors
void Enable_Data_IR(int IR_number, int data){
	if (IR_number>= 1 && IR_number<6){ //Ensure the IR number is within range
		if(IR_number==1){
			IO_expander.digitalWrite(IR_enable_1,data);
		}
		if(IR_number==2){
			IO_expander.digitalWrite(IR_enable_2,data);
		}
		if(IR_number==3){
			IO_expander.digitalWrite(IR_enable_3,data);
		}
		if(IR_number==4){
			IO_expander.digitalWrite(IR_enable_4,data);
		}
		if(IR_number==5){
			IO_expander.digitalWrite(IR_enable_5,data);
		}
	}
}
